...
export const getWalletConnectPayload = <T extends Partial<Payload>>(
  payload: T,
  ...
  return {
    code: payload.command,
    data: payload.data,
    caps: getSigners(payload.caps).flatMap((signer) =>
      signer.clist.map(({ name, args }) => {
    ...
    gasLimit: payload.meta.gasLimit,
    gasPrice: payload.meta.gasPrice,
...
export const signWithWalletConnect =
  (client: Client, session: SessionTypes.Struct) =>
  async <T extends Partial<Payload>>(payload: T): Promise<T> => {
    if (!payload.caps) throw new Error('Caps is required');

    const pactPayload = getWalletConnectPayload(payload);
    ...
    const response = await client
      .request<any>({
        topic: session.topic,
        chainId: `kadena:${payload.networkId}`,
    ...
    return {
      ...payload,
      walletResult: response.body,
    };
  };

export const localRaw =
  ({ preflight = false, signatureValidation = false }) =>
  async <T extends Partial<SignedPayload>>(payload: T) => {
    const response = await fetch(
      ...
      )}/api/v1/local?preflight=${preflight}&signatureValidationt=${signatureValidation}`,
      {
        ...
        body: JSON.stringify(payload.walletResult),
      },
    );
